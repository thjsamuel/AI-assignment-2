...
void SceneAssignment1::Init()
{
    ...
    //for (int i = 0; i < 5; ++i)
    {
        //CCustomer* customer;
        //if (i == 0)
            customer = new CCustomer(entityMgr->GetNextID(), SEAT_1, true, Vector3(145, 10, 0), false);
        //else if (i == 1)
...
void SceneAssignment1::CreateFlock(Vector3 seat_pos)
{
    ...
    {
        const int away_distance = 10; // distance away from the previous group
        ...
        {
            //if (/*customer_list[0]->GetStateInText() == "Group up" || customer_list[0]->GetStateInText() == "Queue up"*/theCustomer->GetLeaderStatus()) // if first customer is queueing up, line up at back of queue
            if (customer_list.back() != nullptr)
            {
        ...
        if (theLeader != theCustomer)
        {
            ...
            theCustomer->AddMember(theLeader);
        }
    ...
    if (flock_to_join >= Group::GROUP_MAX)
    {
        if (customer_list.size() >= 8)
            flock_to_join = Group::GROUP_MAX;
        else if (customer_list.size() < 8)
...
void SceneAssignment1::AssignSeatsToGroup()
{
	...
	if (theLeader && !theLeader->GetHasSeatStatus())
	{
		// Find an empty table
		...
		if (theTable)
		{
			...
			{
				//theTable->GetSeatList()->at(seatIndex)->bUsing = true;
				(*it)->SetTableID(theTable->GetID());
				(*it)->SetSeatPosition(theTable->GetSeatList()->at(seatIndex)->position);
				(*it)->SetHasSeatStatus(true);

				if (seatIndex != theTable->GetNumSeats() - 1)
					seatIndex++;
			}

			// Tell the last customer that he is the last to enter
			//theLeader->GetMembers()->at(theLeader->GetMembers()->size() - 1)->SetLastStatus(true);

			theTable->SetUsingState(true);
			theTable->FillUpSeats();

			// Assign seat to the leader
			theLeader->SetTableID(theTable->GetID());
			theLeader->SetSeatPosition(theTable->GetSeatList()->at(0)->position);
			theLeader->SetHasSeatStatus(true);
		}
		else
		{
			seatIndex = 1;
...
void SceneAssignment1::UpdateTables()
{
	for (std::vector<CTable*>::iterator it = CEntityManager::GetInstance()->GetTableList()->begin(); it != CEntityManager::GetInstance()->GetTableList()->end(); it++)
	{
		(*it)->Update();
...
void SceneAssignment1::GenerateCustomers()
{
	...
	*/
	...
	{
		if (entityMgr->GetEntityFromID(i))
		{
    ...
    if (!bSeat1Taken || !bSeat2Taken || !bSeat3Taken || !bSeat4Taken || !bSeat5Taken || !bSeat6Taken || !bSeat7Taken || !bSeat8Taken || !bSeat9Taken)
    {
        if ((rand() % 800 + 1) == 1)
        {
			...
			if (customer_list.size() > 0)
			{
                if (customer_list[0] != nullptr && customer_list[0]->GetStateInText() == "Queue up") // if first customer is queueing up, line up at back of queue
				{
					Vector3 behind_pos = customer_list.back()->waypoints[0]; // this assumes that the latest customer is queueing up as well and that the new customer should line up behind him by away_distance
					behind_pos.x += away_distance;
					theCustomer->waypoints[0] = behind_pos;
    ...
    //for (int i = 0; i < customer_list.size(); ++i)
    //{
    //    if (customer_list[i]->GetLeaderStatus() && customer_list[i]->GetStateInText() == "Find seat" && customer_list[i]->move_to_front == false)
    //    {
    //        Vector3 front_pos = ENTRANCE; // this assumes that the latest customer is queueing up as well and that the new customer should line up behind him by away_distance
    //        front_pos.x += 25;
    //        if (customer_list[i]->GetPosition() != front_pos)
    //        {
    //            for (int j = i + 1; j < customer_list.size(); ++j)
    //            {
    //                if (customer_list[j]->GetLeaderStatus() && customer_list[j]->GetStateInText() == "Find seat" && customer_list[j]->move_to_front == false)
    //                {
    //                    customer_list[j]->waypoints[0] = front_pos;
    //                    customer_list[j]->GetFSM()->ChangeState(CState_Flock::GetInstance(), 0);
    //                    customer_list[i]->move_to_front = true;
    //                    customer_list[j]->move_to_front = true;
    //                    //front_leader = customer_list[j];
    //                    break;
    //                }
    //            }
    //            break;
...
void SceneAssignment1::DestroyCustomers()
{
    std::vector<CCustomer*>::iterator it = customer_list.begin();
    for (it; it != customer_list.end(); it++)
    {
...
void SceneAssignment1::Update(double dt)
{
    ...
    // Makes sure customer does not wonder out of screen bounds
    for (int i = 0; i < customer_list.size(); ++i)
    {
        if (customer_list[i] != nullptr)
        {
...
void SceneAssignment1::RenderRestaurant()
{
	...
	for (std::vector<CTable*>::iterator it = entityMgr->GetTableList()->begin(); it != entityMgr->GetTableList()->end(); it++)
	{
		...
		{
			modelStack.PushMatrix();
			modelStack.Translate(table->GetPos().x, table->GetPos().y * 1.13, 0);
			...
			{
				Vector3 seatPos = table->GetSeatList()->at(i)->position;
